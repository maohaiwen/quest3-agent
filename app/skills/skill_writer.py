...
请输出改进后的完整内容。"""
    ...
    async def chat(
        ...
        Workflow:
        1. Detect skill type from existing files (or use the type passed from UI)
        2. Based on skill type, determine which files to generate
        3. LLM only generates content, not file plans
        ...
        try:
            # ========== Step 1: Detect skill type from files or UI ==========
            # Priority: UI parameter > existing files > default prompt-only
            ui_provided_type = skill_type is not None
            if not skill_type:
                skill_type = self._detect_skill_type_from_files_content(files_content)
            logger.info(f"Skill type: {skill_type} (from UI: {ui_provided_type})")

            # ========== Step 2: Determine which files to generate based on type ==========
            file_plan = self._get_file_plan_for_type(skill_type, last_user_msg, current_file, files_content)
            files_to_edit = file_plan.get("files", [])
            script_type = file_plan.get("script_type", skill_type or "prompt-only")
    ...
    def _detect_skill_type_from_files_content(self, files_content: Dict[str, str]) -> str:
        """Detect skill type from existing file names."""
        if any(f.endswith('.py') for f in files_content):
            return "python"
        if any(f.endswith('.ps1') for f in files_content):
            return "powershell"
        if any(f.endswith('.sh') for f in files_content):
            return "shell"
        return "prompt-only"

    def _get_file_plan_for_type(
        self,
        skill_type: str,
        user_msg: str,
        ...
        """Determine which files to generate based on the skill type.

        No LLM involved — pure rule-based decision.
        """
        user_msg_lower = user_msg.lower()

        # Check if user is only asking to edit the current file
        editing_current = (
            current_file
            and not any(k in user_msg_lower for k in [
                '完善', '完整', '创建', 'create', 'complete', 'make', 'build',
                '新建', '添加脚本', 'add script',
            ])
        )

        if skill_type == "prompt-only":
            if editing_current and current_file:
                files = [current_file]
            else:
                files = ["skill.md"]
        ...
        elif skill_type == "shell":
            if editing_current and current_file:
                files = [current_file]
            else:
                files = ["skill.md", "main.sh"]

        elif skill_type == "powershell":
            if editing_current and current_file:
                files = [current_file]
            else:
                files = ["skill.md", "main.ps1"]

        else:
            files = ["skill.md"]

        # Add requirements.txt if user mentions dependencies
        ...
        return {
            "files": files,
            "script_type": skill_type or "prompt-only",
            "skill_type": skill_type or "prompt-only",
            "description": "",
            "tags": [],
            "tools": [],
        }
...
- **重要原则**：优先选择prompt-only，除非用户明确需要脚本逻辑（如调用API、数据处理、系统操作等）。纯角色扮演、问答、建议、翻译、写作类技能不需要脚本。
...
现在请输出JSON："""
        ...
        # Fallback to default: prompt-only (just skill.md)
        return {
            "thinking": "使用默认方案：纯提示词型",
            "skill_type": "prompt-only",
            "files": ["skill.md"],
            "script_type": "prompt-only",
            "reason": "默认使用纯提示词型，用户明确需要脚本时再生成"
        }
    ...
    def _parse_file_list(self, response: str, current_file: Optional[str], user_msg: str) -> List[str]:
        ...
        if not files:
            ...
            if current_file and not any(k in user_msg_lower for k in ['完善', '完整', '创建', 'create', 'complete']):
                ...
                # Default to prompt-only: just skill.md, no script
                files = ['skill.md']
                # Only add script files if user explicitly mentions script/code/脚本
                if any(k in user_msg_lower for k in ['脚本', 'script', '代码', 'python', 'main.py', '程序']):
